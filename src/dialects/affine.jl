...
```
"""
function apply(mapOperands; result_0=nothing::Union{Nothing, MLIRType}, map, location=Location())
    results = MLIRType[]
    operands = API.MlirValue[get_value.(mapOperands)..., ]
...
```
"""
function delinearize_index(linear_index, basis; multi_index=nothing::Union{Nothing, Vector{MLIRType}}, location=Location())
    results = MLIRType[]
    operands = API.MlirValue[get_value(linear_index), get_value.(basis)..., ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
    (multi_index != nothing) && push!(results, multi_index...)
...
"""
function for_(lowerBoundOperands, upperBoundOperands, inits; results::Vector{MLIRType}, lowerBoundMap, upperBoundMap, step, region::Region, location=Location())
    results = MLIRType[results..., ]
    operands = API.MlirValue[get_value.(lowerBoundOperands)..., get_value.(upperBoundOperands)..., get_value.(inits)..., ]
    owned_regions = Region[region, ]
    successors = Block[]
    attributes = NamedAttribute[namedattribute("lowerBoundMap", lowerBoundMap), namedattribute("upperBoundMap", upperBoundMap), namedattribute("step", step), ]
    push!(attributes, operandsegmentsizes([length(lowerBoundOperands), length(upperBoundOperands), length(inits), ]))
...
```
"""
function if_(operand_0; results::Vector{MLIRType}, thenRegion::Region, elseRegion::Region, location=Location())
    results = MLIRType[results..., ]
...
```
"""
function load(memref, indices; result::MLIRType, map, location=Location())
...
```
"""
function max(operands; result_0=nothing::Union{Nothing, MLIRType}, map, location=Location())
    results = MLIRType[]
...
```
"""
function min(operands; result_0=nothing::Union{Nothing, MLIRType}, map, location=Location())
    results = MLIRType[]
    operands = API.MlirValue[get_value.(operands)..., ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function parallel(mapOperands; results::Vector{MLIRType}, reductions, lowerBoundsMap, lowerBoundsGroups, upperBoundsMap, upperBoundsGroups, steps, region::Region, location=Location())
    results = MLIRType[results..., ]
    operands = API.MlirValue[get_value.(mapOperands)..., ]
    owned_regions = Region[region, ]
    successors = Block[]
...
instruction cache.
"""
function prefetch(memref, indices; isWrite, localityHint, isDataCache, map, location=Location())
    results = MLIRType[]
    operands = API.MlirValue[get_value(memref), get_value.(indices)..., ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[namedattribute("isWrite", isWrite), namedattribute("localityHint", localityHint), namedattribute("isDataCache", isDataCache), namedattribute("map", map), ]
...
```
"""
function store(value, memref, indices; map, location=Location())
    results = MLIRType[]
...
"""
function vector_load(memref, indices; result::MLIRType, map, location=Location())
    results = MLIRType[result, ]
    operands = API.MlirValue[get_value(memref), get_value.(indices)..., ]
    owned_regions = Region[]
    successors = Block[]
...
"""
function vector_store(value, memref, indices; map, location=Location())
    results = MLIRType[]
    operands = API.MlirValue[get_value(value), get_value(memref), get_value.(indices)..., ]
    owned_regions = Region[]
    successors = Block[]
