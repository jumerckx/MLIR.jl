...
"""
function batch_mmt4d(inputs, outputs; result_tensors::Vector{IR.Type}, region::Region, location=Location())
    results = IR.Type[result_tensors..., ]
    operands = API.MlirValue[get_value.(inputs)..., get_value.(outputs)..., ]
    owned_regions = Region[region, ]
    successors = Block[]
...
them to the same data type as the accumulator/output.
"""
function batch_vecmat(inputs, outputs; result_tensors::Vector{IR.Type}, region::Region, location=Location())
    results = IR.Type[result_tensors..., ]
    operands = API.MlirValue[get_value.(inputs)..., get_value.(outputs)..., ]
    owned_regions = Region[region, ]
    successors = Block[]
...
them to the same data type as the accumulator/output.
"""
function conv_2d_ngchw_gfchw(inputs, outputs; result_tensors::Vector{IR.Type}, strides=nothing, dilations=nothing, region::Region, location=Location())
    results = IR.Type[result_tensors..., ]
    operands = API.MlirValue[get_value.(inputs)..., get_value.(outputs)..., ]
    owned_regions = Region[region, ]
    successors = Block[]
...
"""
function conv_2d_nhwc_fhwc_q(inputs, outputs; result_tensors::Vector{IR.Type}, strides=nothing, dilations=nothing, region::Region, location=Location())
    results = IR.Type[result_tensors..., ]
    operands = API.MlirValue[get_value.(inputs)..., get_value.(outputs)..., ]
    owned_regions = Region[region, ]
    successors = Block[]
