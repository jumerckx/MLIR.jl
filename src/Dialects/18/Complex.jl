...
```
"""
function abs(complex; result=nothing::Union{Nothing, IR.Type}, fastmath=nothing, location=Location())
    results = IR.Type[]
    operands = API.MlirValue[get_value(complex), ]
...
```
"""
function add(lhs, rhs; result=nothing::Union{Nothing, IR.Type}, fastmath=nothing, location=Location())
    results = IR.Type[]
...
```
"""
function angle(complex; result=nothing::Union{Nothing, IR.Type}, fastmath=nothing, location=Location())
    results = IR.Type[]
    operands = API.MlirValue[get_value(complex), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function conj(complex; result=nothing::Union{Nothing, IR.Type}, fastmath=nothing, location=Location())
    results = IR.Type[]
    operands = API.MlirValue[get_value(complex), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function cos(complex; result=nothing::Union{Nothing, IR.Type}, fastmath=nothing, location=Location())
    results = IR.Type[]
    operands = API.MlirValue[get_value(complex), ]
    owned_regions = Region[]
    successors = Block[]
...
function create(real, imaginary; complex::IR.Type, location=Location())
    results = IR.Type[complex, ]
    operands = API.MlirValue[get_value(real), get_value(imaginary), ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
...
```
"""
function div(lhs, rhs; result=nothing::Union{Nothing, IR.Type}, fastmath=nothing, location=Location())
    results = IR.Type[]
    operands = API.MlirValue[get_value(lhs), get_value(rhs), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function exp(complex; result=nothing::Union{Nothing, IR.Type}, fastmath=nothing, location=Location())
    results = IR.Type[]
    operands = API.MlirValue[get_value(complex), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function expm1(complex; result=nothing::Union{Nothing, IR.Type}, fastmath=nothing, location=Location())
    results = IR.Type[]
    operands = API.MlirValue[get_value(complex), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function im(complex; imaginary=nothing::Union{Nothing, IR.Type}, fastmath=nothing, location=Location())
    results = IR.Type[]
    operands = API.MlirValue[get_value(complex), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function log1p(complex; result=nothing::Union{Nothing, IR.Type}, fastmath=nothing, location=Location())
    results = IR.Type[]
    operands = API.MlirValue[get_value(complex), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function log(complex; result=nothing::Union{Nothing, IR.Type}, fastmath=nothing, location=Location())
    results = IR.Type[]
    operands = API.MlirValue[get_value(complex), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function mul(lhs, rhs; result=nothing::Union{Nothing, IR.Type}, fastmath=nothing, location=Location())
    results = IR.Type[]
    operands = API.MlirValue[get_value(lhs), get_value(rhs), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function neg(complex; result=nothing::Union{Nothing, IR.Type}, fastmath=nothing, location=Location())
    results = IR.Type[]
    operands = API.MlirValue[get_value(complex), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function pow(lhs, rhs; result=nothing::Union{Nothing, IR.Type}, fastmath=nothing, location=Location())
    results = IR.Type[]
    operands = API.MlirValue[get_value(lhs), get_value(rhs), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function re(complex; real=nothing::Union{Nothing, IR.Type}, fastmath=nothing, location=Location())
    results = IR.Type[]
    operands = API.MlirValue[get_value(complex), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function rsqrt(complex; result=nothing::Union{Nothing, IR.Type}, fastmath=nothing, location=Location())
    results = IR.Type[]
    operands = API.MlirValue[get_value(complex), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function sign(complex; result=nothing::Union{Nothing, IR.Type}, fastmath=nothing, location=Location())
    results = IR.Type[]
    operands = API.MlirValue[get_value(complex), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function sin(complex; result=nothing::Union{Nothing, IR.Type}, fastmath=nothing, location=Location())
    results = IR.Type[]
    operands = API.MlirValue[get_value(complex), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function sqrt(complex; result=nothing::Union{Nothing, IR.Type}, fastmath=nothing, location=Location())
    results = IR.Type[]
    operands = API.MlirValue[get_value(complex), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function sub(lhs, rhs; result=nothing::Union{Nothing, IR.Type}, fastmath=nothing, location=Location())
    results = IR.Type[]
    operands = API.MlirValue[get_value(lhs), get_value(rhs), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function tan(complex; result=nothing::Union{Nothing, IR.Type}, fastmath=nothing, location=Location())
    results = IR.Type[]
    operands = API.MlirValue[get_value(complex), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function tanh(complex; result=nothing::Union{Nothing, IR.Type}, fastmath=nothing, location=Location())
    results = IR.Type[]
    operands = API.MlirValue[get_value(complex), ]
    owned_regions = Region[]
    successors = Block[]
