...
```
"""
function apply(mapOperands; result_0=nothing::Union{Nothing, IR.Type}, map, location=Location())
    results = IR.Type[]
...
```
"""
function delinearize_index(linear_index, basis; multi_index=nothing::Union{Nothing, Vector{IR.Type}}, location=Location())
    results = IR.Type[]
    operands = API.MlirValue[get_value(linear_index), get_value.(basis)..., ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
    !isnothing(multi_index) && push!(results, multi_index...)
...
"""
function for_(lowerBoundOperands, upperBoundOperands, inits; results::Vector{IR.Type}, lowerBoundMap, upperBoundMap, step, region::Region, location=Location())
...
```
"""
function if_(operand_0; results::Vector{IR.Type}, thenRegion::Region, elseRegion::Region, location=Location())
    results = IR.Type[results..., ]
...
```
"""
function load(memref, indices; result::IR.Type, map, location=Location())
...
```
"""
function max(operands; result_0=nothing::Union{Nothing, IR.Type}, map, location=Location())
    results = IR.Type[]
...
```
"""
function min(operands; result_0=nothing::Union{Nothing, IR.Type}, map, location=Location())
    results = IR.Type[]
    operands = API.MlirValue[get_value.(operands)..., ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function parallel(mapOperands; results::Vector{IR.Type}, reductions, lowerBoundsMap, lowerBoundsGroups, upperBoundsMap, upperBoundsGroups, steps, region::Region, location=Location())
    results = IR.Type[results..., ]
    operands = API.MlirValue[get_value.(mapOperands)..., ]
    owned_regions = Region[region, ]
    successors = Block[]
...
"""
function vector_load(memref, indices; result::IR.Type, map, location=Location())
    results = IR.Type[result, ]
    operands = API.MlirValue[get_value(memref), get_value.(indices)..., ]
    owned_regions = Region[]
    successors = Block[]
