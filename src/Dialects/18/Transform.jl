...
"""
function apply_conversion_patterns(target::Value; legal_ops=nothing, illegal_ops=nothing, legal_dialects=nothing, illegal_dialects=nothing, partial_conversion=nothing, preserve_handles=nothing, patterns::Region, default_type_converter_region::Vector{Region}, location=Location())
    results = IR.Type[]
    operands = Value[target, ]
...
"""
function collect_matching(root::Value; results::Vector{IR.Type}, matcher, location=Location())
    results = IR.Type[results..., ]
    operands = Value[root, ]
    owned_regions = Region[]
    successors = Block[]
...
"""
function foreach_match(root::Value; updated::IR.Type, restrict_root=nothing, matchers, actions, location=Location())
...
"""
function get_operand(target::Value; result::IR.Type, raw_position_list, is_inverted=nothing, is_all=nothing, location=Location())
    results = IR.Type[result, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
...
"""
function get_parent_op(target::Value; parent::IR.Type, isolated_from_above=nothing, allow_empty_results=nothing, op_name=nothing, deduplicate=nothing, nth_parent=nothing, location=Location())
...
"""
function get_result(target::Value; result::IR.Type, raw_position_list, is_inverted=nothing, is_all=nothing, location=Location())
    results = IR.Type[result, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
...
"""
function include(operands::Vector{Value}; results::Vector{IR.Type}, target, failure_propagation_mode, location=Location())
    results = IR.Type[results..., ]
...
"""
function sequence(root=nothing::Union{Nothing, Value}; extra_bindings::Vector{Value}, results::Vector{IR.Type}, failure_propagation_mode, body::Region, location=Location())
    results = IR.Type[results..., ]
...
"""
function split_handle(handle::Value; results::Vector{IR.Type}, pass_through_empty_handle=nothing, fail_on_payload_too_small=nothing, overflow_result=nothing, location=Location())
    results = IR.Type[results..., ]
    operands = Value[handle, ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function gpu_map_nested_forall_to_threads(target::Value; result::IR.Type, block_dims=nothing, sync_after_distribute=nothing, warp_size=nothing, location=Location())
    results = IR.Type[result, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
...
"""
function structured_bufferize_to_allocation(target::Value; allocated_buffer::IR.Type, new_ops::IR.Type, memory_space=nothing, memcpy_op=nothing, alloc_op=nothing, bufferize_destination_only=nothing, emit_dealloc=nothing, location=Location())
...
Returns a silenceable failure if the pattern application failed.
"""
function structured_flatten_elementwise(target::Value; transformed::IR.Type, location=Location())
    results = IR.Type[transformed, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
...
"""
function structured_match(target::Value; results::IR.Type, ops=nothing, interface=nothing, op_attrs=nothing, filter_result_type=nothing, filter_operand_types=nothing, location=Location())
    results = IR.Type[results, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
...
"""
function structured_pad(target::Value; padded::IR.Type, pad::IR.Type, copy::IR.Type, padding_values=nothing, padding_dimensions=nothing, pad_to_multiple_of=nothing, pack_paddings=nothing, transpose_paddings=nothing, copy_back_op=nothing, location=Location())
...
"""
function structured_promote(target::Value; transformed::IR.Type, operands_to_promote=nothing, use_full_tile_buffers=nothing, use_full_tiles_by_default=nothing, use_alloca=nothing, memory_space=nothing, mapping=nothing, alignment=nothing, location=Location())
    results = IR.Type[transformed, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function structured_tile_reduction_using_for(target::Value; fill_op::IR.Type, split_linalg_op::IR.Type, combining_linalg_op::IR.Type, for_op::IR.Type, tile_sizes=nothing, location=Location())
...
```
"""
function structured_tile_reduction_using_forall(target::Value; fill_op::IR.Type, split_linalg_op::IR.Type, combining_linalg_op::IR.Type, forall_op::IR.Type, num_threads=nothing, tile_sizes=nothing, mapping=nothing, location=Location())
...
produces a definite failure.
"""
function structured_tile_using_for(target::Value, dynamic_sizes::Vector{Value}; tiled_linalg_op::IR.Type, loops::Vector{IR.Type}, static_sizes=nothing, interchange=nothing, scalable_sizes=nothing, location=Location())
...
Returns a silenceable failure if the pattern application failed.
"""
function structured_transpose_conv2d(target::Value; transformed::IR.Type, location=Location())
    results = IR.Type[transformed, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
...
"""
function structured_vectorize_children_and_apply_patterns(target::Value; transformed::IR.Type, vectorize_padding=nothing, vectorize_nd_extract=nothing, flatten_1d_depthwise_conv=nothing, disable_multi_reduction_to_contract_patterns=nothing, disable_transfer_permutation_map_lowering_patterns=nothing, location=Location())
    results = IR.Type[transformed, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
...
"""
function match_structured_body(operand_handle::Value; reduction_position=nothing, passthrough=nothing, elementwise=nothing, contraction=nothing, location=Location())
    results = IR.Type[]
    operands = Value[operand_handle, ]
    owned_regions = Region[]
    successors = Block[]
...
"""
function memref_alloca_to_global(alloca::Value; getGlobal::IR.Type, global_::IR.Type, location=Location())
    results = IR.Type[getGlobal, global_, ]
    operands = Value[alloca, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
...
requested.
"""
function loop_forall_to_for(target::Value; transformed::Vector{IR.Type}, location=Location())
    results = IR.Type[transformed..., ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
...
"""
function loop_fuse_sibling(target::Value, source::Value; fused_loop::IR.Type, location=Location())
...
"""
function loop_peel(target::Value; peeled_loop::IR.Type, remainder_loop::IR.Type, peel_front=nothing, fail_if_already_divisible=nothing, location=Location())
