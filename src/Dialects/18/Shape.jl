...
```
"""
function assuming_all(inputs; result=nothing::Union{Nothing, IR.Type}, location=Location())
    results = IR.Type[]
    operands = API.MlirValue[get_value.(inputs)..., ]
    owned_regions = Region[]
    successors = Block[]
...
ready to execute.
"""
function assuming(witness; results::Vector{IR.Type}, doRegion::Region, location=Location())
...
concat([], [4,5,6]) -> [4,5,6]
"""
function concat(lhs, rhs; result::IR.Type, location=Location())
    results = IR.Type[result, ]
    operands = API.MlirValue[get_value(lhs), get_value(rhs), ]
    owned_regions = Region[]
    successors = Block[]
...
"""
function dim(value, index; extent=nothing::Union{Nothing, IR.Type}, location=Location())
    results = IR.Type[]
    operands = API.MlirValue[get_value(value), get_value(index), ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
...
```
"""
function from_extents(extents; shape=nothing::Union{Nothing, IR.Type}, location=Location())
    results = IR.Type[]
    operands = API.MlirValue[get_value.(extents)..., ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
    !isnothing(shape) && push!(results, shape)
...
invalid arguments.
"""
function value_of(arg; result::IR.Type, location=Location())
    results = IR.Type[result, ]
    operands = API.MlirValue[get_value(arg), ]
    owned_regions = Region[]
    successors = Block[]
