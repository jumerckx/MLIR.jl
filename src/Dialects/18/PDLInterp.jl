...
```
"""
function apply_constraint(args::Vector{Value}; results::Vector{IR.Type}, name, isNegated=nothing, trueDest::Block, falseDest::Block, location=Location())
...
```
"""
function apply_rewrite(args::Vector{Value}; results::Vector{IR.Type}, name, location=Location())
    results = IR.Type[results..., ]
    operands = Value[args..., ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function create_range(arguments::Vector{Value}; result::IR.Type, location=Location())
