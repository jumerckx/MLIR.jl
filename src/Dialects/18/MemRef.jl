...
```
"""
function atomic_rmw(value, memref, indices; result=nothing::Union{Nothing, IR.Type}, kind, location=Location())
    results = IR.Type[]
...
```
"""
function generic_atomic_rmw(memref, indices; result::IR.Type, atomic_body::Region, location=Location())
...
in these contexts.
"""
function load(memref, indices; result=nothing::Union{Nothing, IR.Type}, nontemporal=nothing, location=Location())
    results = IR.Type[]
    operands = API.MlirValue[get_value(memref), get_value.(indices)..., ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function alloc(dynamicSizes, symbolOperands; memref::IR.Type, alignment=nothing, location=Location())
...
chosen.
"""
function alloca(dynamicSizes, symbolOperands; memref::IR.Type, alignment=nothing, location=Location())
    results = IR.Type[memref, ]
    operands = API.MlirValue[get_value.(dynamicSizes)..., get_value.(symbolOperands)..., ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function extract_aligned_pointer_as_index(source; aligned_pointer=nothing::Union{Nothing, IR.Type}, location=Location())
    results = IR.Type[]
    operands = API.MlirValue[get_value(source), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function extract_strided_metadata(source; base_buffer=nothing::Union{Nothing, IR.Type}, offset=nothing::Union{Nothing, IR.Type}, sizes=nothing::Union{Nothing, Vector{IR.Type}}, strides=nothing::Union{Nothing, Vector{IR.Type}}, location=Location())
    results = IR.Type[]
    operands = API.MlirValue[get_value(source), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function memory_space_cast(source; dest::IR.Type, location=Location())
    results = IR.Type[dest, ]
    operands = API.MlirValue[get_value(source), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function rank(memref; result_0=nothing::Union{Nothing, IR.Type}, location=Location())
    results = IR.Type[]
    operands = API.MlirValue[get_value(memref), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function realloc(source, dynamicResultSize=nothing; result_0::IR.Type, alignment=nothing, location=Location())
...
```
"""
function reinterpret_cast(source, offsets, sizes, strides; result::IR.Type, static_offsets, static_sizes, static_strides, location=Location())
    results = IR.Type[result, ]
...
```
"""
function transpose(in; result_0::IR.Type, permutation, location=Location())
    results = IR.Type[result_0, ]
...
```
"""
function view(source, byte_shift, sizes; result_0::IR.Type, location=Location())
    results = IR.Type[result_0, ]
...
```
"""
function subview(source, offsets, sizes, strides; result::IR.Type, static_offsets, static_sizes, static_strides, location=Location())
    results = IR.Type[result, ]
    operands = API.MlirValue[get_value(source), get_value.(offsets)..., get_value.(sizes)..., get_value.(strides)..., ]
    owned_regions = Region[]
    successors = Block[]
